...
CreateDescriptorSetLayout(VkDevice device, const std::vector<VkDescriptorSetLayoutBinding> &bindings) {
    ...
    VULKAN_RHI_SAFE_CALL(vkCreateDescriptorSetLayout(device, &createInfo, nullptr, &result));

    return result;
}

VkPipelineLayout
CreatePipelineLayout(const VkDevice &device, const std::vector<VkDescriptorSetLayout> &descriptorSetLayouts) {
    ...
    VULKAN_RHI_SAFE_CALL(vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &result));
    return result;
}

// TODO: remove this
VkRenderPass CreateDummyRenderPass(VkDevice device, VkFormat surfaceFormat, VkFormat depthFormat) {
    ...
    VkRenderPass result;
    VULKAN_RHI_SAFE_CALL(vkCreateRenderPass(device, &renderPassInfo, nullptr, &result));
...
VulkanGraphicsPipeline::VulkanGraphicsPipeline(VkDevice device, VkExtent2D extent, VkFormat surfaceFormat, VkFormat depthFormat,
    ...
    layoutHandle = CreatePipelineLayout(deviceHandle, descriptorSetLayouts);

    vk::RenderPass renderPass = CreateDummyRenderPass(deviceHandle, surfaceFormat, depthFormat);
    ...
    VULKAN_RHI_SAFE_CALL(vkCreateGraphicsPipelines(deviceHandle, nullptr, 1, &pipelineInfo, nullptr, &pipelineHandle));
}

VulkanGraphicsPipeline::~VulkanGraphicsPipeline() {
    vkDestroyPipeline(deviceHandle, pipelineHandle, nullptr);

    vkDestroyPipelineLayout(deviceHandle, layoutHandle, nullptr);
...
const std::vector<VkDescriptorSetLayout> &VulkanGraphicsPipeline::DescriptorLayouts() const {
